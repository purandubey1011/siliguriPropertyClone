...
const getFilteredProperties = catchAsyncErrors(async (req, res, next) => {
  ...
  if (filters.propertyTypes) {
    const selectedTypes = [];
    if (filters.propertyTypes.apartment)
      selectedTypes.push("Flat/Apartment", "Apartment");
    if (filters.propertyTypes.land)
      selectedTypes.push("Residential Land", "Land");
    if (filters.propertyTypes.farm)
      selectedTypes.push("Farm House", "Farmhouse");
  ...
  if (filters.postedBy) {
    ...
    if (filters.postedBy.feature)
      selectedPosters.push("Feature Dealer", "featured dealer");
  ...
  if (filters.localities) {
    ...
    if (filters.localities.hosur)
      selectedLocalities.push("Hosur Road", "Hosur");
    ...
    if (selectedLocalities.length > 0) {
      ...
      const localityRegexes = selectedLocalities.map(
        (loc) => new RegExp(loc.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "i")
      );
  ...
  if (filters.facingDirection) {
    ...
    if (filters.facingDirection.north)
      selectedDirections.push("North", "north");
    if (filters.facingDirection.south)
      selectedDirections.push("South", "south");
  ...
  if (filters.purchaseType) {
    ...
    if (filters.purchaseType.resale)
      selectedPurchaseTypes.push("Resale", "resale");
    if (filters.purchaseType.newBooking)
      selectedPurchaseTypes.push("New Booking", "new booking", "New");
...
const getAllUsers = catchAsyncErrors(async (req, res, next) => {
  const users = await userModel.find({}).populate("properties").lean();
